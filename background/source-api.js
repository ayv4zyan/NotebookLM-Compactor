...
const RPC_GET_USER_SETTINGS = "ZwVcOc";
...
function buildBatchExecuteUrl(sourcePath, rpcId, blVersion) {
...
function resolveSourcePath(notebookId, sourcePath) {
  if (typeof sourcePath === "string" && sourcePath.trim().length > 0) {
    return sourcePath.trim();
  }
  if (typeof notebookId === "string" && notebookId.trim().length > 0) {
    return `/notebook/${notebookId.trim()}`;
  }
  throw new Error("batchExecute requires notebookId or sourcePath");
}
...
export function buildGetUserSettingsParams() {
  return [null, [1, null, null, null, null, null, null, null, null, null, [1]]];
}

function buildGetUserSettingsBody(atToken) {
  return buildFReqBody(RPC_GET_USER_SETTINGS, buildGetUserSettingsParams(), atToken);
}
...
async function batchExecute({ notebookId, sourcePath, rpcId, body, blVersion }) {
  const now = Date.now();
  ...
  const url = buildBatchExecuteUrl(
    resolveSourcePath(notebookId, sourcePath),
    rpcId,
    blVersion
  );
...
async function getUserSettings({ atToken, blVersion }) {
  const body = buildGetUserSettingsBody(atToken);
  const innerData = await batchExecute({
    sourcePath: "/",
    rpcId: RPC_GET_USER_SETTINGS,
    body,
    blVersion,
  });
  return extractAccountLimits(innerData);
}
  ...
  try {
    switch (action) {
      ...
      case "getUserSettings": {
        const limits = await getUserSettings({ atToken, blVersion });
        return {
          success: true,
          notebookLimit: limits.notebookLimit,
