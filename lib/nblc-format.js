...
(function (root, factory) {
  ...
  if (typeof module !== "undefined" && module.exports) {
    ...
    // Content scripts must use window (not globalThis) — Firefox keeps them separate.
    const target =
    ...
    target.NBLC.format = api;
  }
})(typeof globalThis !== "undefined" ? globalThis : this, function () {
  const NBLC_VERSION = 1;
  const COMPACTOR_ID = "NotebookLM-Compactor/1.0.1";
  ...
  function sourceFromParts(meta, content) {
    ...
    return enrichSourceForDecompact({
      index: meta.index,
      title: meta.title,
      type: meta.type,
      url: meta.url,
  ...
  function extractUrlFromContent(content) {
    ...
    const metaLine = content.match(/^\s*url:\s*(https?:\/\/\S+)/im);
    if (metaLine) return metaLine[1].replace(/[),.]+$/, "");

    const sourceLine = content.match(/^Source:\s+\[([^\]]+)\]\(([^)]+)\)/m);
    if (sourceLine) return sourceLine[2];
    ...
    const bare = content.match(
      /https?:\/\/(?:www\.)?(?:youtube\.com\/[^\s)\]]+|youtu\.be\/[^\s)\]]+)/i
    );
    return bare ? bare[0].replace(/[),.]+$/, "") : null;
  }

  function enrichSourceForDecompact(source) {
    const title = source?.title || "Untitled";
    const content = source?.content ?? "";
    let type = source?.type || "unknown";
    let url = source?.url || null;

    if (!url) url = extractUrlFromContent(content);

    if (url && isYoutubeUrl(url)) {
      if (!type || type === "unknown") type = "youtube";
    } else if (url && (!type || type === "unknown")) {
      type = "web";
    }
    ...
    return {
      ...
      type,
      url,
      original_id: source.original_id || null,
      content,
    };
  }

  /**
   * Choose decompact upload RPC: URL restore for youtube/web when url metadata exists.
   * Falls back to URL in section content (NotebookLM often strips meta lines on round-trip).
   * Spec: docs/NBLC-FORMAT.md — Decompact upload mapping
   */
  function resolveDecompactUpload(source) {
    const type = source?.type || "unknown";
    const title = source?.title || "Untitled";
    const content = source?.content ?? "";
    const url = source?.url || extractUrlFromContent(content) || null;

    if (url && (type === "youtube" || isYoutubeUrl(url))) {
      return { action: "addYoutube", url };
    }

    if (url && type === "web") {
      return { action: "addUrl", url };
    }

    return { action: "addText", title, content };
  }

  function describeDecompactMethod(source) {
    const plan = resolveDecompactUpload(source);
    switch (plan.action) {
      case "addYoutube":
        return "YouTube URL";
      case "addUrl":
