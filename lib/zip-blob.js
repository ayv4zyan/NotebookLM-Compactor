 ...
 * Backup zips are larger than JSZip DEFLATE output; see docs/FIREFOX.md.
 */
(function (root, factory) {
  ...
  function chunkEndIndex(str, start) {
    let end = Math.min(start + ENCODE_CHAR_CHUNK, str.length);
  ...
  async function encodeUtf8Chunked(str) {
    ...
    const parts = [];
    let totalLen = 0;
    for (let i = 0; i < safe.length; ) {
      const end = chunkEndIndex(safe, i);
      const slice = safe.slice(i, end);
