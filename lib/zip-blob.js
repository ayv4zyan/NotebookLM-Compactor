...
(function (root, factory) {
  ...
  const YIELD_CHAR_THRESHOLD = 256 * 1024;
  ...
  async function createZipBlob(files) {
    ...
    for (const [name, content] of entries) {
      ...
      if (nameStr.length + contentStr.length >= YIELD_CHAR_THRESHOLD) {
        await yieldToMain();
      }

      const nameBytes = encoder.encode(nameStr);
      const dataBytes = encoder.encode(contentStr);
      const checksum = await crc32Chunked(dataBytes);
      const size = dataBytes.length;
      const gpbf = nameNeedsUtf8Flag(String(name)) ? UTF8_GPBF : 0;
      ...
      localView.setUint16(8, gpbf, true);
