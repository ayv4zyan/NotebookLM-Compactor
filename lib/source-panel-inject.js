(function () {
  ...
  const i18n = {
    en: {
      ...
      selectOneNblcSource: "Select exactly one NBLC source to decompact",
    },
    zh: {
      compactSelected: "压缩选中的来源",
      selectSourcesToCompact: "请选择要压缩的来源",
      decompactSelected: "解压 NBLC 压缩包",
      selectOneNblcSource: "请选择一个 NBLC 来源进行解压",
    },
  };
  ...
  function onDecompactClick() {
    ...
    if (!nblcSource) {
      alert(t("selectOneNblcSource"));
      return;
    }

    window.dispatchEvent(
      new CustomEvent("nblc-decompact", {
        detail: { sourceId: nblcSource.id, title: nblcSource.title },
      })
    );
  }
  ...
  function updateDecompactButtonState() {
    ...
    const nblcSource = getSelectedNblcSource();
    const enabled = nblcSource !== null;

    decompactButton.disabled = !enabled;
    decompactButton.style.opacity = enabled ? "1" : "0.4";
    decompactButton.style.cursor = enabled ? "pointer" : "not-allowed";
  }
