...
### Operation stuck or errors

1. Open `about:debugging` → **This Firefox**.
2. Find **NotebookLM Compactor** → click **Inspect** next to the background script (event page).
3. Also open the NotebookLM tab → **Inspect** (F12) → Console — most `batchexecute` calls run in the content script.
4. Check both consoles for errors during Compact or Decompact.
5. If the background page shows as stopped, trigger the operation again from the notebook page.

Long Decompact runs (many sources) poll `waitForSourceReady` per source — this can take several minutes.
