...
Extract in the content script and pass as `atToken` on each API call. The content-script `batchexecute` bridge (`content/batchexecute-bridge.js`) runs requests in-page with session cookies. Background worker is a fallback path only.
...
## RPC: GET_USER_SETTINGS — `ZwVcOc` (account limits)

**Implemented as:** `getUserSettings` in `background/source-api.js`.

Fetches per-account limits advertised by NotebookLM (`GetOrCreateAccount`). Used by the Compact modal quota panel to show `current / source_limit` and headroom. Reference: [notebooklm-py `_settings.py`](https://github.com/teng-lin/notebooklm-py/blob/main/src/notebooklm/_settings.py).

**Account-scoped:** `source-path` is `/`, not `/notebook/{id}`. No `notebookId` in params.
...
Limits live at `result[0][1]` (parsed by `extractAccountLimits()` in `background/rpc-parse.js`):

| Index | Field |
|-------|-------|
| 1 | `notebook_limit` |
| 2 | `source_limit` (per-notebook source cap for this account) |

Returns `{ notebookLimit, sourceLimit }` — either may be `null` if absent or schema drift.

### UI behavior

- Called once when the user opens Compact (user-initiated, non-blocking).
- On success: quota panel shows `N / limit`, progress bar, and slots remaining after compact.
- On failure or missing `source_limit`: falls back to current count only (no hardcoded plan assumptions).

---

## Extension message protocol

Modals call `NBLC.runtimeMessaging.sendSourceApi(body)`, which prefers the in-page `batchexecute` client and falls back to background messaging:

```javascript
// Content → Background (fallback path)
chrome.runtime.sendMessage({
  type: "source-api",
  body: {
    action: "getContent" | "delete" | "addText" | "addUrl" | "addYoutube" | "getNotebook" | "getUserSettings",
    sourceId,
    sourceIds,
    notebookId, // not required for getUserSettings
    atToken,
...
| `getUserSettings` | `ZwVcOc` | ✅ | Account `source_limit` / `notebook_limit`; `source-path: /` |
