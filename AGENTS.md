...
- **Background:** Chrome MV3 `background.service_worker` (bundled IIFE); Firefox MV3 `background.scripts` (event page). Generate dist trees with `node scripts/build-extension.mjs`. Source: `background/index.js` (ES module).
- **API routing:** `lib/runtime-messaging.js` sends `batchexecute` from the content script (cookies); background is fallback + storage sweep.
...
CI runs build + all five tests on every push/PR to `main`.
...
3. ✅ Manual test signed off — [docs/MANUAL_TEST_CHECKLIST.md](./docs/MANUAL_TEST_CHECKLIST.md) (Chrome + Firefox, 2026-06-20).
...
1. Max pasted-text source size — stress-test with 30+ long transcripts (not yet verified at scale).
2. ~~Whether to extract `bl` from page HTML proactively~~ — **resolved:** yes, with hardcoded fallback.
3. ~~Per-account source count limit~~ — **resolved:** `getUserSettings` (`ZwVcOc`) + `extractAccountLimits()`; UI falls back to count-only on parse failure.