(function () {
  ...
  function getQuotaMath() {
    ...
    const headroomAfter = NOTEBOOK_SOURCE_LIMIT - after;
    return { current, selected, after, freed, headroomAfter };
  }

  function renderQuotaBar(fillCount) {
    const pct = Math.min(
      100,
      Math.round((fillCount / NOTEBOOK_SOURCE_LIMIT) * 100)
    );
    return `
  ...
  function renderQuotaPanel() {
    const { current, selected, after, freed, headroomAfter } = getQuotaMath();
      ...
      return `
        <div class="nblc-quota-panel">
          ...
          <p class="nblc-quota-value">
            ${current}
            <span class="nblc-quota-limit">/ ~${NOTEBOOK_SOURCE_LIMIT}</span>
          </p>
          ${renderQuotaBar(current)}
          <p class="nblc-quota-hint">Select 2 or more sources to free quota slots.</p>
        </div>
      `;
    }
    ...
    const headroomLine =
      freed > 0 && headroomAfter >= 0
        ? `<p class="nblc-quota-meta">${headroomAfter} slot${headroomAfter === 1 ? "" : "s"} remaining before ~${NOTEBOOK_SOURCE_LIMIT}-source limit</p>`
        : "";
